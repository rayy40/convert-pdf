...
const Documents = () => {
  ...
  const { fileDetails, isRenaming, setIsModalOpen, setHasAccount } =
  ...
  const handleUploadFiles = (e) => {
    ...
    uploadFiles("documents", Array.from(files), user);
  };
  ...
  const deleteFiles = () => {
    ...
    const deletePromises = selectedFiles.flatMap((file) => [
      deleteObject(
        ref(storage, `/${user?.uid}/upload/${file?.metadata?.name}`)
      ),
      deleteObject(
        ref(storage, `/${user?.uid}/modified/${file?.metadata?.name}`)
      ),
    ]);
  ...
  const handleMerge = () => {
    ...
    convertFiles("merge-pdf", files[0]?.metadata, urls, undefined, user);
  };
  ...
  if (isUserLoading) {
    return (
      <div className="loading-container_wrapper">
        <Loading />
      </div>
    );
  }
